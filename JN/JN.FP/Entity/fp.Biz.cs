...
namespace JN.FP.Entity
{
    ...
    public class fp : fp<fp> { }
    
    /// <summary></summary>
    public partial class fp<TEntity> : Entity<TEntity> where TEntity : fp<TEntity>, new()
    {
        #region 对象操作﻿
        static fp()
        {
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("开始初始化{0}fp数据……", typeof(TEntity).Name);
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("完成初始化{0}fp数据！", typeof(TEntity).Name);
        //}
        #endregion
        ...
        /// </summary>
        public Decimal ZJE {
            get { return this.金额 + this.金额1 + this.金额2 + this.金额3; }
        }
        #endregion

        #region 扩展查询﻿
        /// <summary>根据发票编号查找</summary>
        /// <param name="__发票编号"></param>
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static TEntity FindBy发票编号(String __发票编号)
        {
            if (Meta.Count >= 1000)
                return Find(_.发票编号, __发票编号);
            else // 实体缓存
                return Meta.Cache.Entities.Find(_.发票编号, __发票编号);
            // 单对象缓存
            //return Meta.SingleCache[__发票编号];
        }
        #endregion
