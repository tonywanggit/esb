...
namespace JN.Studio.Entity
{
    ...
    public class UserInfo : UserInfo<UserInfo> { }
    
    /// <summary></summary>
    public partial class UserInfo<TEntity> : Entity<TEntity> where TEntity : UserInfo<TEntity>, new()
    {
        #region 对象操作﻿
        static UserInfo()
        {
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("开始初始化{0}UserInfo数据……", typeof(TEntity).Name);
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("完成初始化{0}UserInfo数据！", typeof(TEntity).Name);
        //}
        #endregion
        ...
        /// <returns></returns>
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public static TEntity FindByLoginName(String loginName)
        {
            if (Meta.Count >= 1000)
                return Find(_.LoginName, loginName);
            else // 实体缓存
                return Meta.Cache.Entities.Find(_.LoginName, loginName);
            // 单对象缓存
            //return Meta.SingleCache[oid];
        }
        #endregion
