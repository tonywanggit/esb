...
namespace JN.Studio.Entity
{
    ...
    /// <summary></summary>
    public partial class Database<TEntity> : Entity<TEntity> where TEntity : Database<TEntity>, new()
    {
        #region 对象操作﻿
        static Database()
        {
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("开始初始化{0}Database数据……", typeof(TEntity).Name);
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("完成初始化{0}Database数据！", typeof(TEntity).Name);
        //}
        #endregion
            ...
            get
            {
                switch (DatabaseType)
                {
                    case "Microsoft SQL Server":
                        return String.Format("Server={1};Database={0};Integrated Security=False;User ID={2};Password={3};"
                            , DatabaseName
                            , Server
                            , UserName
                            , Password
        ...
        /// <returns></returns>
        public static TEntity FindByConnName(String connName)
        {
            if (Meta.Count >= 1000)
                return Find(_.ConnName, connName);
            else 
                return Meta.Cache.Entities.Find(_.ConnName, connName);
        }
        #endregion
