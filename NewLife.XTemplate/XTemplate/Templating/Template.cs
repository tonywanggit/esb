    ...
    public partial class Template : DisposeBase
    {
        ...
        public static string BaseClassName
        {
            get
            {
                ...
                {
                    _BaseClassName = Config.GetConfig<string>("XTemplate.BaseClassName", string.Empty);
                }
        ...
        public static bool Debug
        {
            get
            {
                if (!_Debug.HasValue)
                {
                    _Debug = new bool?(Config.GetConfig<bool>("XTemplate.Debug", false));
                }
                return _Debug.Value;
            }
            set
            {
                _Debug = new bool?(value);
            }
        }
        ...
        public static List<string> Imports
        {
            get
            {
                ...
                _Imports = list;
                string[] configSplit = Config.GetConfigSplit<string>("XTemplate.Imports", null);
                if ((configSplit != null) && (configSplit.Length > 0))
                {
                    list.AddRange(configSplit);
                }
                string[] strArray2 = new string[] { "System", "System.Collections", "System.Collections.Generic", "System.Text" };
                ...
                dictionary.Add("XCommon", new string[] { "XCommon" });
                dictionary.Add("XControl", new string[] { "XControl" });
                dictionary.Add("System.Web", new string[] { "System.Web" });
                dictionary.Add("System.Xml", new string[] { "System.Xml" });
                System.Reflection.Assembly[] assemblies = AppDomain.CurrentDomain.GetAssemblies();
                foreach (string str2 in dictionary.Keys)
                {
                    foreach (System.Reflection.Assembly assembly in assemblies)
                    {
                        if (assembly.FullName.StartsWith(str2 + ","))
                        {
                            strArray2 = dictionary[str2];
                            if ((strArray2 != null) && (strArray2.Length > 0))
                            {
                ...
                ICollection<string> is2 = new HashSet<string>();
                is2.Add("XCode");
                is2.Add("NewLife.Core");
                is2.Add("NewLife.CommonEntity");
                foreach (string str4 in is2)
                {
                    foreach (System.Reflection.Assembly assembly2 in assemblies)
                    {
                        if (assembly2.FullName.StartsWith(str4 + ","))
                        {
                            foreach (Type type in assembly2.GetTypes())
                            {
                                string str5 = type.Namespace;
                                if ((!string.IsNullOrEmpty(str5) && (str5.StartsWith("XCode") || str5.StartsWith("NewLife"))) && !list.Contains(str5))
                                {
                                    list.Add(str5);
                                }
                            }
        ...
        public static List<string> References
        {
            get
            {
                ...
                string[] configSplit = Config.GetConfigSplit<string>("XTemplate.References", null);
                if ((configSplit != null) && (configSplit.Length > 0))
                {
                    foreach (string str in configSplit)
                    {
                        list.Add(str);
        ...
        {
            string str = fileName;
            return str.Replace(@"\", "_").Replace("/", "_").Replace(".", "_").Replace(Path.VolumeSeparatorChar, '_').Replace(Path.PathSeparator, '_');
        }
        ...
        protected static string Hash(string str)
        {
            ...
            return BitConverter.ToString(MD5.Create().ComputeHash(Encoding.UTF8.GetBytes(str))).Replace("-", null);
        }
        #endregion
